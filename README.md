...
See `docs/product-contract.md` for the durable product contract: supported default and enhanced provider modes, output category boundaries, unavailable-context handling, and non-goals. See `docs/reporting.md` for the renderer contract used by JSON, table, and Markdown reports.
...
This mirrors the `Python checks (3.12)` and `Python checks (3.13)` CI jobs and runs lint, typecheck, tests, Markdown docs link checks, dependency/security checks, and installed CLI smoke checks. See [Local check parity with CI](docs/local-check-parity.md) for the exact tox-to-CI mapping, required GitHub check names, and common setup blockers.
